...
> **These are design estimates, not measured values.** The Sprint-3 live-fetch
> smoke test (JP-47, 2026-05-09) was blocked — see **First successful run**
> below. Replace this table once a live run completes.
...
**Status: DEFERRED — CloudFront IP block (2026-05-09)**

Sprint-3 story JP-47 attempted a live fetch from the Hetzner host
(`ubuntu-32gb-fsn1-2`, Hetzner AX41). Every request to
`https://www.courtlistener.com/api/bulk-data/opinions/tax.tar.gz` returned
HTTP 403 from CloudFront: _"Request blocked."_

Attempts made (all blocked):
- `User-Agent: JudicialPredict-Ingest/0.1` → 403
- `User-Agent: Mozilla/5.0 (X11; Linux x86_64)` → 403
- `https://storage.courtlistener.com/bulk-data/opinions/tax.tar.gz` → 404
- CourtListener REST v3 opinions endpoint (`/api/rest/v3/opinions/?court__id=tax`) → empty results (auth required)
- CourtListener REST v4 opinions endpoint (`/api/rest/v4/opinions/`) → HTTP 401 "Authentication credentials were not provided"

**Root cause:** Hetzner Frankfurt data-center IP ranges are blocked by
CourtListener's CloudFront WAF. This is an IP-based block, not a User-Agent
or rate-limit issue. The REST v4 API also requires a Bearer token for all
reads (CourtListener changed access policy).

**What was completed in JP-47:**
- Release binary built successfully (`target/release/ingest-fetcher`)
- All 8 unit + fixture tests pass (5 lib + 3 parse_test)
- `fetch.rs` fixed: switched from `reqwest::get()` (no User-Agent) to an
  explicit `reqwest::Client` with `User-Agent: JudicialPredict-Ingest/0.1`
  plus two new unit tests (`bulk_url_uses_tax_court_identifier`,
  `bulk_url_encodes_court_identifier`)
- `bulk_url()` extracted as a pure function for testability

**To unblock the live run, one of:**
1. Register a CourtListener API account at https://www.courtlistener.com/sign-in/,
   obtain a token, and add `Authorization: Token <token>` to `download_dump`
   (the v4 REST bulk endpoint may work with a token even from Hetzner IPs).
2. Run the ingest from a non-datacenter IP (e.g. a residential or AWS IP that
   is not in CourtListener's WAF block list).
3. Contact CourtListener (https://free.law/contact/) to whitelist the Hetzner
   egress IP for bulk access.

Once a live run succeeds, fill in this table:

| Date | Court | Dump (MB) | Download | Parse | Upsert | Rows |
|------|-------|----------:|---------:|------:|-------:|-----:|
| TBD  | tax   |       TBD |      TBD |   TBD |    TBD |  TBD |
