...
Available at `/admin/audit_log/auditlogentry/`.

**Who can view:** any active operator (admin, viewer, or super).
**Who can write:** nobody — the table is append-only (Rust inserts only).

### RLS semantics

| Operator role | DB alias | Rows visible |
|---|---|---|
| `admin` / `viewer` | `default` (jp_app, RLS enforced) | Own tenant only |
| `super` | `admin_super` (jp_admin, BYPASSRLS) | All tenants |

The admin UI does **not** add a Python-side `get_queryset` filter for
`tenant_id` — Postgres RLS is the single enforcement point.  This avoids
double-filtering for tenant-scoped operators and keeps super-operator
behaviour correct.

### Filters and search

- **Action** — drop-down list filter (e.g. `predict.invoke`, `tenant_settings.override_change`).
- **Reason code** — drop-down list filter.
- **Table name** — drop-down list filter.
- **Date** — built-in date hierarchy (Today / Past 7 days / This month / This year).
- **Search** — free-text across `subject_id`, `action`, `row_pk`.

### Screenshot

`docs/screenshots/audit-log-admin.png` — placeholder; actual screenshot Sprint 5.

### Sprint-5 follow-ups

- Export to CSV.
- Row-level link from audit entry to source case/operator record.
- Full-text search on `payload_hash` (once added to audit_log schema).
- Explicit date-range pickers (replace built-in Today/This week shortcuts).
...
- ~~**Audit-log viewer**~~: **done** (S4.9) — read-only `ModelAdmin` for `audit_log`.
  CSV export and explicit date pickers remain Sprint-5 items.
